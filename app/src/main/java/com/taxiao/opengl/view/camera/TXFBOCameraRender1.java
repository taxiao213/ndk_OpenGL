 ...
 */
public class TXFBOCameraRender1 {
    private String TAG = TXFBOCameraRender1.this.getClass().getSimpleName();
    ...
    private int bitmapTexture1;
    private int bitmapTexture2;

    public TXFBOCameraRender1(Context context) {
        this.mContext = context;
        bitmap = ShaderUtils.createTextImage("猜测恶是11112", 50, "#ff0000", "#00000000", 10);
        bitmap1 = ShaderUtils.createTextImage("2222", 50, "#ff0000", "#00000000", 10);
        bitmap2= ShaderUtils.createTextImage("婚事魔王", 50, "#ff0000", "#00000000", 10);
...
//-------
        vertexData[16] = 0.8f - w;
        vertexData[17] = -0.8f;

        vertexData[18] = 0.8f;
        vertexData[19] = -0.8f;

        vertexData[20] = 0.8f - w;
        vertexData[21] = -0.7f;
...
//-------
        vertexData[24] = -(0.8f - w);
        vertexData[25] = -0.8f;

        vertexData[26] = -0.8f;
        vertexData[27] = -0.8f;

        vertexData[28] = -(0.8f - w);
        vertexData[29] = -0.7f;
    ...
    private void initOpenGLES() {
        ...
        if (program > 0) {
            ...
            bitmapTexture1 = ShaderUtils.loadBitmapTexture(bitmap1);
            bitmapTexture2 = ShaderUtils.loadBitmapTexture(bitmap2);
        }
    }
    ...
    private void renderFrame(int imageTexure) {
        if (program > 0) {
            ...
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, bitmapTexture1);
            GLES20.glEnableVertexAttribArray(av_position);
            GLES20.glVertexAttribPointer(av_position, 2, GLES20.GL_FLOAT, false, 8, 64);
            GLES20.glEnableVertexAttribArray(af_position);
            GLES20.glVertexAttribPointer(af_position, 2, GLES20.GL_FLOAT, false, 8, vertexData.length * 4);
            GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4);
            ...
            GLES20.glEnableVertexAttribArray(av_position);
            GLES20.glVertexAttribPointer(av_position, 2, GLES20.GL_FLOAT, false, 8, 96);
            GLES20.glEnableVertexAttribArray(af_position);
            GLES20.glVertexAttribPointer(af_position, 2, GLES20.GL_FLOAT, false, 8, vertexData.length * 4);
            GLES20.glDrawArrays(GLES20.GL_TRIANGLE_STRIP, 0, 4);
